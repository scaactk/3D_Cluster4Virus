...
int update_new(std::vector<bool>& processed, std::vector<float>& reachability_distance, std::vector<int>& neighbour_idx,
    ...
    for (int i = 0; i < neighbour_idx.size(); i++) {
        if (processed[neighbour_idx[i]] == false) {
            ...
            if (reachability_distance[neighbour_idx[i]] == 0) {
                // point is not in seeds
...
static int extract_id_new(MyPointCloud& cloud, const std::vector<int>& ordered_sequence,
                          const std::vector<float>& output_dist, float filter) {
    int clusterID = 0;
    bool pre = true;
    bool cur = true;
...
static bool write2file_new(const std::string& folderPath, const std::vector<int>& ordered_sequence,
...
static bool write2file_idx_point(MyPointCloud& cloud, const std::string& folderPath, const std::string& filename_no_ext,
                                 const std::vector<int>& ordered_sequence, const std::vector<float>& output_dist) {
    std::ofstream file(folderPath + "/" + filename_no_ext + "_idx_point.csv"); // auto create new file
    std::cout << folderPath + "/" + filename_no_ext + "_idx_point.csv" << std::endl;
    if (!file.is_open()) {
        std::cout << "Unable to open _idx_point file" << std::endl;
        return false;
    }
...
std::tuple<std::vector<int>, std::vector<float>> optics_new(MyPointCloud& cloud,
                                                            const pcl::KdTreeFLANN<PointT>& treeFlann, float eps,
                                                            int min, const std::string& folderPath,
    ...
    write2file_idx_point(cloud, folderPath, filename_no_ext, ordered_sequence, result_distance);
